...
setup_director_database_packages = ["pyyaml","pandas","requests"]
install_packages(setup_director_database_packages)
...
from ops_database_operations import create_database,return_conn,insert_pd_df_in_table
...
def create_director_database(director_name,tmdb_keys_path,sql_user,sql_pwd):
    ...
    create_database(sql_user,sql_pwd,db_name)

    uri = 'mysql+pymysql://' + sql_user + ':' + sql_pwd + '@' + 'mysql' + ':' + '3306' + '/' + db_name
    conn = return_conn(uri)
    insert_pd_df_in_table(conn,movies_from_director_df,'movies',if_exists_method = 'replace')
    ...
    insert_pd_df_in_table(conn,actors_from_director_df,'actors_by_movie',if_exists_method = 'replace')
    ...
    insert_pd_df_in_table(conn,actors_df,'actors',if_exists_method = 'replace')
