...
def create_recognizer(embeddings_folder, recognizer_path, model_name = None, C=1.0, kernel='linear', probability=True, logger = None):
    ...
    if model_name is None:
        ...
        model_name = files.get_latest_model_name(os.path.join(embeddings_folder,embeddings_index_file), logger = logger)
        if model_name == 'embeddings_0':
          logger.info(f'No model found, could not create recognizer.')
          return
        embeddings_index = files.read_json_file(os.path.join(embeddings_folder,embeddings_index_file), logger = logger)
    ...
    embeddings_data = files.read_pickle_file(model_data[0]['embeddings_path'],logger = logger)
    le = LabelEncoder()
    ...
    files.create_pickle_file(recognizer_dict,os.path.join(recognizer_path,model_name+'_recognizer.pickle'),logger = logger)
...
def predict_probabilities(target_embeddings,embeddings_folder,recognizer_folder,array_format = False, model_name = None,logger = None):
    ...
    recognizer = files.read_pickle_file(recognizer_path, logger = logger)
